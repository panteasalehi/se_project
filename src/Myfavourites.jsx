...
export const Myfavourites = (props) => {
      ...
      useEffect(() => {
        ...
        const getItems = async () => {
          try{
            ...
            let response = await axios.get("http://localhost:8000/favs?email=" + email);
    ...
    return (
        <div >  
          ...
          <ul className="flex flex-col">
          <h2 className="text-3xl my-4">لیست علاقه مندی  ها </h2>
          {
          items.map((item) => (
          ...
          <button style= {{width : "60%", marginBottom:"5%"}} onClick = {() => props.onFormSwitch("Myposts",email)}  >اگهی ها </button> 
            ...
            <button  style= {{width : "60%", marginBottom:"5%"}}  onClick = {() => props.onFormSwitch("notes",email)}>یادداشت ها </button> 
